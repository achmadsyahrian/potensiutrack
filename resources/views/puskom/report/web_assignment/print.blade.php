...
<body>
         ...
         <thead>
            <tr>
               ...
               <td class="header-cell" style="width: 30%;">
                  <strong>NO. DOKUMEN</strong><br>F-/SPMI/27-03-03
               </td>
            </tr>
            <tr>
               <td colspan="2" rowspan="2" class="header-cell" style="font-size: 13px;">
                  <strong>JUDUL</strong><br>PENUGASAN PEMBUATAN DAN MAINTENANCE WEB DEVELOPMENT DAN APLIKASI SISTEM INFORMASI
               </td>
               <td class="">
                  <span style="width: 120px; display: inline-block"> Tanggal Terbit </span> : 14 Mei 2024
               </td>
            </tr>
            <tr>
               <td class="">
                  <span style="width: 120px; display: inline-block"> Tanggal Efektif </span> : 16 Mei 2024
               </td>
            </tr>
            <tr>
   ...
   @foreach ($chunkedData as $index => $dataChunk)
      @php $pageNumber = ($index * 10) + 1; @endphp
      <div class="container" style="position:absolute; bottom:0; width:100%; left:0;">
         ...
         <table class="table-data" style="margin-top:15px;">
            <thead>
               <tr>
                  <th rowspan="2">No.</th>
                  <th rowspan="2">Programmer</th>
                  <th rowspan="2">Aplikasi</th>
            ...
            <tbody>
               @foreach ($dataChunk as $item)
                  <tr>
                     ...
                     <td>
                        {!! $item->assignment_type == 'maintenance' ? '<div style="font-family: DejaVu Sans, sans-serif;">&#10003;</div>' : '' !!}
                     </td>
                     <td>
                        {!! $item->assignment_type == 'development' ? '<div style="font-family: DejaVu Sans, sans-serif;">&#10003;</div>' : '' !!}
                     </td>
                     <td>{{ \Carbon\Carbon::parse($item->finish_date)->locale('id_ID')->isoFormat('D MMMM YYYY') }}</td>
                     <td style="padding:0;">
                        <img src="{{ asset('storage/'.$item->programmer_signature) }}" alt="Paraf Programmer" width="50">
                     </td>
                     <td style="padding:0;">
                        <img src="{{ asset('storage/'.$item->kabag_signature) }}" alt="Paraf Kabag" width="50">
                     </td>
                  </tr>
               @endforeach
            </tbody>
