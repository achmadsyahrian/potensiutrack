               ...
               <tbody>
                  @forelse ($users as $item)
                      <tr>
                        ...
                        <td><span class="text-muted">{{ ($users->currentPage() - 1) * $users->perPage() + $loop->iteration }}</span></td>
                        <td data-label="Name">
                           <div class="d-flex py-1 align-items-center">
                              @if (!$item->photo) 
                                 <span class="avatar me-2" style="background-image: url({{ asset('image/avatar/defaul-profile.jpg') }})"></span> 
                              @else
                                 <span class="avatar me-2" style="background-image: url({{ asset('storage/' . $item->photo) }})"></span> 
                              @endif
                              <div class="flex-fill">
                                 <div class="font-weight-medium">{{ $item->name }}</div>
                                 <div class="text-muted"><a href="mailto:{{ $item->email }}" class="text-reset">{{ $item->email ?? '---' }}</a></div>
                              </div>
                           </div>
                        </td>
                        <td>
                           <span class="badge bg-success me-1"></span> {{ $item->nip }}
                        </td>
                        <td>
                           {{ $item->username }}
                        </td>
                        <td>
                           <div class="btn-list justify-content-end flex-nowrap">
                              <a href="{{ route('lecturers.edit', ['lecturer' => $item->id]) }}" class="btn btn-outline-info">
                                 Edit
                             </a>
                        ...
                        <x-confirm-modal route="{{ route('lecturers.destroy', ['lecturer' => $item->id]) }}" method='delete' id='{{ $item->id }}'></x-confirm-modal>
                     </tr>
                  @empty
                     <tr>
                        <td colspan="5">
...
@include('components.lecturers.modal')
