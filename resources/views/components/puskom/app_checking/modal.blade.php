<div class="modal modal-blur fade" id="modal-report" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <form action="{{ route('puskom.appchecking.store') }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('post')
                        ...
                        <div class="col-lg-12">
                            <div class="mb-3">
                                ...
                                <select type="text" class="form-select @error('web_app_id') is-invalid @enderror"
                                    name="web_app_id" id="select-optgroups" value="">
                                    <option selected disabled>Pilih aplikasi</option>
                                    @foreach ($apps as $item)
                                    <option value="{{ $item->id }}" {{ old('web_app_id')==$item->id ? 'selected' : ''
                                        }}>{{ $item->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-lg-6">
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('month') is-invalid @enderror" name="month">
                                    ...
                                    @for ($i = 1; $i <= 12; $i++)
                                        <option value="{{ $i }}" {{ old('month') == $i ? 'selected' : '' }}>{{ Carbon::create()->month($i)->translatedFormat('F') }}</option>
                                    @endfor
                                </select>
                                <x-invalid-feedback field='month'></x-invalid-feedback>
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('year') is-invalid @enderror" name="year">
                                    ...
                                    @for ($tahun = $tahun_awal; $tahun <= $tahun_akhir; $tahun++)
                                        <option value="{{ $tahun }}" {{ old('year') == $tahun ? 'selected' : '' }}>{{ $tahun }}</option>
                                    @endfor
                                </select>
                                <x-invalid-feedback field='year'></x-invalid-feedback>
                            </div>
                        </div>
