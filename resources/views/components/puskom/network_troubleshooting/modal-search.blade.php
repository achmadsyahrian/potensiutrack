<div class="modal modal-blur fade " id="modal-search" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form action="{{ route('puskom.networkdev.index') }}" method="get">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-12">
                            <div class="mb-3">
                                ...
                                <select type="text" class="form-select" name="search_reporter">
                                    <option selected disabled>Pilih pegawai</option>
                                    @foreach ($employees as $item)
                                        <option value="{{ $item->id }}" {{ request('search_reporter') == $item->id ? 'selected' : '' }}>
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                  </select>
